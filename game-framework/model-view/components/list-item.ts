...
export class ListItem extends Component {
    //图标
    @property({ type: Sprite, tooltip: DEV ? '图标' : "" }) icon: Sprite = null!;
    //标题
    @property({ type: Node, tooltip: DEV ? '标题' : "" }) title: Node = null!;
    //选择模式
    @property({
    ...
    @property({
        ...
        visible(this: ListItem) { return this.selectedMode > SelectedType.NONE }
    })
    selectedFlag: Node = null!;
    //被选择的SpriteFrame
    @property({
        type: SpriteFrame, tooltip: DEV ? '被选择的SpriteFrame' : "",
        visible(this: ListItem) { return this.selectedMode == SelectedType.SWITCH }
    })
     ...
     */
    createEvt(component: Component, handlerName: string, node: IGameFramework.Nullable<Node> = null) {
        if (!component.isValid)
            return;//有些异步加载的，节点以及销毁了。
        (<any>component)['comName'] = (<any>component)['comName'] || (<any>component).name.match(/\<(.*?)\>/g).pop().replace(/\<|>/g, '');
        let evt = new EventHandler();
        evt.target = node || component.node;
        evt.component = (<any>component)['comName'];
